...
pub fn get_redis_con(ip_port: &str) -> Result<Connection, &'static str> {
	...
	let client = redis::Client::open(redis_url.as_str()).expect(format!("Can't connect to: {}",
																		redis_url)
																		.as_str());
...
pub fn load_config(config_path: Option<&str>) {
	dotenv::from_path(Path::new(config_path.unwrap_or("dashboard.env")))
		.expect(format!("Loading config: \"{:?}\" FAILED", config_path).as_str())
}

pub fn from_config(key: &str) -> String {
	env::var(&key).expect(format!("Set value for key: \"{}\" in config.", &key).as_str())
}
